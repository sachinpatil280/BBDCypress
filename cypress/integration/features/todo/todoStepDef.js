...
Then('Validate list with {string} items are saved', function (val) {
    if (val == 1) {
        cy.get(`.todo-list li:nth-child(1) label`).and('contain', this.data.singleItemList)
    }
    else if (val === 'multiple') {
        var len = this.data.multipleItemList.length
        for (let i = 0; i < len; i++) {
            cy.get(`.todo-list li:nth-child(${i + 1}) label`).and('contain', this.data.multipleItemList[i])
        }
...
When('I create todo list with {string} items', function (val) {
    if (val == 1) {
        cy.get('.new-todo').type(this.data.singleItemList).type('{enter}')
    }
    else if (val === 'multiple') {
...
Then('I remove item from todo list', () => {
    cy.get('.todo-list li:nth-child(1)').find('.destroy').invoke('show').click()
})
...
Then('Validate completed tasks are removed', () => {
    cy.get('.todo-list li').filter('.completed').should('have.length', 0)
})
...
Then('I update the title of task', function () {
    cy.get('.editing').clear().type(this.data.updatedValue).type('{enter}')
})

Then('Validate updated entry', function () {
    cy.contains(this.data.updatedValue).should('be.visible').and('have.length', 1)
})
